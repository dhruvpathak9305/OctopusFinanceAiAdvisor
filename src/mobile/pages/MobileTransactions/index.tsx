...
const transformSupabaseTransaction = (supabaseTransaction: SupabaseTransaction): Transaction => {
  try {
    return {
      ...
      type: (supabaseTransaction.type as 'income' | 'expense' | 'transfer') || 'expense',
      icon: getTransactionIcon(supabaseTransaction.type, supabaseTransaction.icon),
      date: supabaseTransaction.date || new Date().toISOString().split('T')[0],
    };
  } catch (error) {
    console.error('Error transforming transaction:', error, supabaseTransaction);
...
const getTransactionIcon = (type: string, customIcon?: string | null): string => {
...
const MobileTransactions: React.FC<MobileTransactionsProps> = ({ className = "" }) => {
    ...
    try {
      ...
      switch (selectedSort) {
        ...
        case 'Smallest Amount':
          ...
          break;
        case 'Income':
          sortedTransactions = sortedTransactions.filter(t => t.type === 'income');
          break;
        case 'Expense':
          sortedTransactions = sortedTransactions.filter(t => t.type === 'expense');
          break;
        case 'Transfer':
          sortedTransactions = sortedTransactions.filter(t => t.type === 'transfer');
          break;
  ...
  const summaryData = {
    ...
    expenses: transactions.filter(t => t && t.type === 'expense').reduce((sum, t) => sum + Math.abs(t.amount || 0), 0),
    net: transactions.filter(t => t).reduce((sum, t) => sum + (t.amount || 0), 0),
  };
                        ...
                        onPress={() => {
                          Alert.alert(
                            ...
                            [
                              ...
                              { text: 'Delete', style: 'destructive', onPress: () => handleDeleteTransaction(transaction.id) }
                            ]
                          );
                        }}
                      >
