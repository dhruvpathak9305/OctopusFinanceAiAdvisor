...
const MODEL_CONFIGS: Record<string, ModelConfig> = {
  ...
  'deepseek-r1': {
    ...
    apiKey: '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',
...
export class LLMService {
  ...
  private static async callLlamaAPI(
    ...
    if (!response.ok) {
      if (response.status === 404) {
        throw new Error(`Model "${config.model}" not found. Please check if the model is available.`);
      }
  ...
  private static async callDeepSeekAPI(
    ...
    const headers: Record<string, string> = {
      ...
      'HTTP-Referer': typeof window !== 'undefined' ? window.location.origin : 'https://octopus-finance.ai',
    ...
    if (!response.ok) {
      ...
      console.error('OpenRouter/DeepSeek API Error:', response.status, errorText);
      ...
      if (response.status === 402) {
        throw new Error('OpenRouter API credits exhausted. Please check your account balance.');
      }
  ...
  static getAvailableModels(): Array<{ label: string; value: string; provider: string }> {
    return [
